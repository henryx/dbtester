...
func (db *Postgres) getAuthors(tx *sql.Tx, authors []gjson.Result) []int {
	var err error
	var authorId int
...
func (db *Postgres) getPublishers(tx *sql.Tx, publishers []gjson.Result) []int {
	var publisherId int
...
func (db *Postgres) getGenres(tx *sql.Tx, genres []gjson.Result) []int {
	var genreId int
...
func (db *Postgres) loadAuthors(tx *sql.Tx) {
	var err error
...
func (db *Postgres) loadEditions(tx *sql.Tx) {
	var err error
...
func (db *Postgres) addAuthor(j string) {
	var err error
...
func (db *Postgres) addEdition(tx *sql.Tx, j string) {
	var editionId int
	var err error
...
func (db *Postgres) addISBNs(editionId int, isbn10s []gjson.Result, isbn13s []gjson.Result) {
	var err error
...
func (db *Postgres) addEditionAuthors(editionId int, authors []int) {
	var err error
...
func (db *Postgres) addEditionPublishers(editionId int, publishers []int) {
	var err error
...
func (db *Postgres) addEditionGenres(editionId int, genres []int) {
	var err error
...
func (db *Postgres) prepareStatements(tx *sql.Tx) {
	var err error

	insAuthors := "INSERT INTO authors(key, name, revision, birth, death) VALUES($1, $2, $3, $4, $5)"
	insEditions := "INSERT INTO editions(key, title, subtitle, format, publish_date, edition, description, pages) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id"
	insPublishers := "INSERT INTO publishers(name) VALUES ($1) RETURNING id"
	insGenres := "INSERT INTO genres(name) VALUES ($1) RETURNING id"
	insIsbn10 := "INSERT INTO editions_isbn10 (edition_id, isbn10) VALUES ($1, $2)"
	insIsbn13 := "INSERT INTO editions_isbn13 (edition_id, isbn13) VALUES ($1, $2)"
	insEditionAuthors := "INSERT INTO editions_authors(edition_id, author_id) VALUES($1, $2)"
	insEditionPublishers := "INSERT INTO editions_publishers(edition_id, publisher_id) VALUES($1, $2)"
	insEditionGenres := "INSERT INTO editions_genres(edition_id, genre_id) VALUES($1, $2)"
...
func (db *Postgres) closeStatements() {
